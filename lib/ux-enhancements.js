...
function recordDraftSnapshot() {
  return JSON.stringify({
    ...
    feedMode: value('#feedModeSeg'),
    amount: $('#feedAmount').value,
    duration: $('#nurseDuration').value,
    eliminationKind: value('#eliminationKindSeg'),
    urineStatus: value('#urineStatusSeg'),
    stoolColor: value('#stoolColorSeg'),
    stoolForm: value('#stoolFormSeg'),
    abnormalType: value('#abnormalTypeSeg'),
    severity: value('#severitySeg')
  });
}

function attemptCloseModal() {
  if (recordBaseline && recordDraftSnapshot() !== recordBaseline && !confirm('已修改记录内容，确定放弃本次修改吗？')) return;
  closeModal();
}

const recordCloseHelpers = `let recordBaseline='';
${recordDraftSnapshot.toString()}
${attemptCloseModal.toString()}
`;
