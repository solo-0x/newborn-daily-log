...
function getDb() {
  if (!databaseClient) {
    ...
    const sql=neon(url);
    databaseClient={
      prepare(statement){return {bind(...params){const query=placeholders(statement);return {
      ...
      }},async run(){return {results:await sql.query(statement,[])}}}},
...
async function exportRecords(env, user) {
  ...
  return json({ version: 3, storage: 'neon-postgres', exportedAt: new Date().toISOString(), exportedBy: user.display_name, records: (result.results || []).map(dbRecordToClient) });
}
