...
const TEMP_SESSION_SECONDS = 60 * 60 * 12;
...
const DEFAULT_SETTINGS = Object.freeze({
  ...
  formulaReferenceMode: 'system', customFormulaMin: 60, customFormulaMax: 90, feedIntervalMinutes: 120
});

const FEED_INTERVAL_OPTIONS = Object.freeze([60, 90, 120, 150, 180, 210, 240]);
...
export function sanitizeSettings(raw, strict) {
  ...
  const requestedFeedInterval = Number(source.feedIntervalMinutes ?? DEFAULT_SETTINGS.feedIntervalMinutes);
  const feedIntervalMinutes = FEED_INTERVAL_OPTIONS.includes(requestedFeedInterval) ? requestedFeedInterval : DEFAULT_SETTINGS.feedIntervalMinutes;
  ...
  if (strict && !FEED_INTERVAL_OPTIONS.includes(requestedFeedInterval)) throw Object.assign(new Error('喂奶提醒间隔不正确'), { status: 400 });
  ...
  return { appTitle: appTitle || DEFAULT_SETTINGS.appTitle, babyName, birthDate, timezone, defaultFeedMode, quickAmounts,
    inheritLastRecord, formulaReferenceMode, customFormulaMin, customFormulaMax, feedIntervalMinutes };
}
...
async function login(request, env) {
  ...
  const sessionPolicy = getSessionPolicy(body?.remember === true);
  ...
  await env.DB.prepare('INSERT INTO sessions(token_hash,user_id,expires_at,created_at,last_seen_at) VALUES(?,?,?,?,?)')
    .bind(tokenHash, user.id, nowSec + sessionPolicy.seconds, now, now).run();
  return json({ authenticated: true, user: publicUser(user) }, 200, {
    'Set-Cookie': `${SESSION_COOKIE}=${token}; Path=/; HttpOnly; Secure; SameSite=Strict${sessionPolicy.cookieMaxAge}`
  });
}

export function getSessionPolicy(remember) {
  return remember
    ? { seconds: REMEMBER_SESSION_SECONDS, cookieMaxAge: `; Max-Age=${REMEMBER_SESSION_SECONDS}` }
    : { seconds: TEMP_SESSION_SECONDS, cookieMaxAge: '' };
}
...
async function getAuth(request, env) {
  ...
  if (!row) return null;
  // Lightweight activity marker; the selected login expiry remains fixed.
